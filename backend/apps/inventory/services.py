...
def decrease_stock(inventory, amount):
    ...
    with transaction.atomic():
        ...
        previous_available = locked.available_quantity
        locked.quantity -= amount
        _save_locked(locked, "quantity")
        if previous_available > locked.low_stock_threshold and locked.available_quantity <= locked.low_stock_threshold:
...
def adjust_stock(inventory, quantity_change):
    ...
    with transaction.atomic():
        ...
        previous_available = locked.available_quantity
        previous_threshold = locked.low_stock_threshold
        was_low = previous_available <= previous_threshold
        locked.quantity = new_quantity
        _save_locked(locked, "quantity")
        if not was_low and locked.available_quantity <= locked.low_stock_threshold:
            notify_low_stock(locked)
        return locked
